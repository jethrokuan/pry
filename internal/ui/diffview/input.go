...
func (m Model) handleKey(msg tea.KeyMsg) (Model, tea.Cmd) {
	...
	if m.comments.cursor >= 0 && m.nav.focus == FocusDiff {
		switch {
		...
		case "e":
			return m.editSelectedComment()
		case "d":
			return m.deleteSelectedComment()
		}

		// Other keys: deselect and fall through to normal handling
		m.comments.cursor = -1
	}
